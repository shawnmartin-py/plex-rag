...
def make_mock_client(*, exists: bool = True, vector_size: int = 3072) -> MagicMock:
    ...
    client.get_collection.return_value.config.params.vectors = VectorParams(size=vector_size, distance=Distance.COSINE)
    return client
...
def test_connect_raises_when_server_unreachable():
    with patch("app.services.recommender_vector_store.QdrantClient", side_effect=ConnectionError("refused")):
...
def test_connect_raises_when_collection_missing():
    mock_client = make_mock_client(exists=False)
    with patch("app.services.recommender_vector_store.QdrantClient", return_value=mock_client):
        with pytest.raises(QdrantUnavailableError, match="does not exist"):
            connect_vector_store("http://localhost:6333", "media_items", MagicMock())


def test_connect_raises_when_vector_size_mismatched():
...
def test_connect_returns_vector_store_when_healthy():
    mock_client = make_mock_client()
    with patch("app.services.recommender_vector_store.QdrantClient", return_value=mock_client):
        store = connect_vector_store("http://localhost:6333", "media_items", StubEmbeddings())
...
def test_load_synopsis_documents_builds_documents_from_scroll_results():
    mock_point = MagicMock()
    mock_point.payload = {"page_content": "Title: Parasite", "metadata": {"imdb_id": "tt6751668", "title": "Parasite"}}
...
def test_load_synopsis_documents_filters_by_synopsis_embedding_type():
    mock_vector_store = MagicMock()
