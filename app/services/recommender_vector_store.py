...
def connect_vector_store(url: str, collection_name: str, embeddings: GoogleGenerativeAIEmbeddings) -> QdrantVectorStore:
    ...
    try:
        ...
        raise QdrantUnavailableError(
            f"Could not reach Qdrant at {url}. Is the plex-ingest Docker container running?"
        ) from e
    if not exists:
        raise QdrantUnavailableError(
            f"Qdrant collection '{collection_name}' does not exist at {url}. Run the plex-ingest pipeline first."
        )
    vectors_config = client.get_collection(collection_name).config.params.vectors
    if not isinstance(vectors_config, VectorParams):
        raise QdrantUnavailableError(
            f"Qdrant collection '{collection_name}' uses named/multi-vector config, expected a single "
            f"unnamed vector (gemini-embedding-001) — schema mismatch between plex-rag and plex-ingest."
        )
    actual_size = vectors_config.size
    if actual_size != VECTOR_SIZE:
        raise QdrantUnavailableError(
            f"Qdrant collection '{collection_name}' has vector size {actual_size}, expected {VECTOR_SIZE} "
            "(gemini-embedding-001) — embedding model mismatch between plex-rag and plex-ingest."
        )
    return QdrantVectorStore(client=client, collection_name=collection_name, embedding=embeddings)


def load_synopsis_documents(vector_store: QdrantVectorStore, collection_name: str) -> list[Document]:
    ...
    points, _ = vector_store.client.scroll(
        ...
        scroll_filter=Filter(must=[FieldCondition(key="metadata.embedding_type", match=MatchValue(value="synopsis"))]),
        limit=10_000,
        with_payload=True,
        with_vectors=False,
    )
