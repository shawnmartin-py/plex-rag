...
class HyDEVectorRetriever(CandidateRetriever):
    ...
    _prompt = ChatPromptTemplate.from_messages(
        [
            (
                "human",
                (
                    "Write a dense expert film profile (4-6 sentences) describing the "
                    "ideal film for "
                    "this request. Use precise critical vocabulary: exact subgenre "
                    "labels, cinematic "
                    "movements, director names and influences, tone and mood "
                    "descriptors, thematic "
                    "keywords, narrative structure, emotional register, visual style, "
                    "and pacing. "
                    "Every word should serve as a retrieval signal for finding real "
                    "films that match. "
    ...
    def __init__(
        ...
        self._filter = Filter(
            must=[
                FieldCondition(
                    key="metadata.embedding_type", match=MatchValue(value="enriched")
                )
            ]
        )
    ...
    _prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                (
                    "You are a film expert with deep knowledge of cinema. From the "
    ...
    def __init__(
        ...
        self._filter = (
            Filter(
                must=[
                    FieldCondition(
                        key="metadata.embedding_type",
                        match=MatchValue(value="enriched"),
                    )
                ]
            )
            if filter_by_type
